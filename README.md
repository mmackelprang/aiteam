...
- 📋 **[`docs/implementation-plan.md`](docs/implementation-plan.md)** — the working plan: component placement across dev computer / NAS / appserver (grounded in the `mmackelprang/homelab` repo's conventions, §1.0), review findings F1–F19 (Google Chat per-agent identities, project-onboarding-as-configuration, the Beads + claude-mem memory-layer watch-list, the chat-gateway extraction, and more), the staged plan for both systems, and the decision list D1–D10 (all resolved): pilot repos RTest / homelab / FamilyWorkspace, harness pilot FamilyWorkspace, git-as-transport, Tier-0 allowlists, subscription-auth first, Chat plumbing in `mmackelprang/chat-gateway`.
- ✅ **Doc sets complete** — the agentic-team-framework proposal is in place at `harness/docs/proposal.md` (2026-07-23); every Task 0 document for both systems now exists.
