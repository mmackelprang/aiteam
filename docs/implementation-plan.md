...
**The one memory-layer decision (amends F17).** At **H-Task 9, on F13 evidence**, a single Board decision compares **Paperclip's native context packet vs Beads `bd prime` vs claude-mem injection** for the §8 re-context problem. Until then both tools are watch-list; neither is installed. *Amended 2026-08-08 (F21): persistent sessions + cross-session messaging join as a fourth candidate — solving §8 by not discarding context rather than re-injecting it; same gate, same evidence bar.* *Amended 2026-08-09 (F22): every candidate is evaluated against the refinement-discipline criteria (evidence-backed diffs, review gate, versioned rollback, immutable base prompt); the Paperclip-native candidate's concrete shape is git-versioned per-role refinement files.*
...
### F21 — Claude Code cross-session messaging: fits the harness, gated adoption (research 2026-08-08) → D12

Claude Code v2.1.224+ ships GA cross-session messaging (`ListAgents`/`SendMessage`): same-machine sessions message each other bidirectionally over per-session Unix sockets (OS-user-scoped, container-partitioned); headless `-p` workers can receive; the inbox socket is a documented seam for scripts/hooks to post into a live session; inbound messages carry built-in untrusted-input protections (no permission approval, no config changes, commands inert). Full write-up: **`harness/docs/research-cross-session-messaging.md`**. The appserver team (six roles, one host, one user) is the ideal topology. Three leverage cases, ascending: **L1** wake the raising agent when its review item is answered (seconds instead of next-heartbeat — the H-Task 5 cron posts to the session socket); **L2** role→role handoff pushes (message = coordination, never assignment — Paperclip keeps what/whether, the F17 discipline); **L3** persistent sessions with message-driven turns, attacking §8/F13 re-context overhead by *not discarding context* — joins the H-Task 9 memory bake-off as a fourth candidate (amends F17/F18). Non-negotiables regardless: heartbeats guarantee / messages accelerate (no delivery guarantee, restarts drop everything); a message audit log before any messaging ships (F18 gate-6 principle); explicit Tier-0 posture now — deny `SendMessage`+`ListAgents` and `crossSessionInbound: refuse` in the role presets — because an unconfigured posture stalls headless sessions on held messages (the F10 failure mode). The separate experimental **agent teams** feature duplicates Paperclip's orchestrator role — watch-list only. Adoption scope/timing → **D12**.

### F22 — prime-agent's "continual harness" patterns: steal the governance, not the runtime (research 2026-08-09) → D13

Deep-read of `PrimeIntellect-ai/prime-agent` (MIT autonomous-agent runtime: persistent IPython kernel, daemon sessions, heartbeats, a2a messaging) — full write-up in **`harness/docs/research-prime-agent-patterns.md`**, including source-level mechanics of its refinement engine. The runtime itself is disqualified (unsandboxed model-generated Python with user permissions, by its own docs; and a second orchestrator beside Paperclip). Three steals: **S1** — its *refinement discipline* (small evidence-backed diffs, never wholesale rewrites; a second-model review gate rejecting one-off noise; versioned append-only history; first-class rollback; immutable base prompt; local/global scope isolation) is F18 gate 6 made concrete, and becomes **acceptance criteria for whichever candidate wins the H-Task 9 memory bake-off**. **S2** — the Paperclip-native bake-off candidate gains a concrete shape: **git-versioned per-role refinement files** (agent-owned `memory.md` beside an immutable human-authored role CLAUDE.md; diffs are commits, history is `git log`, rollback is `git revert`, the review gate is a lead/human-approved commit) — the only candidate whose what-did-the-agent-see audit is free. **S3** — the convention "**budget exhaustion ≠ success**; a passed quality gate certifies only what it verifies": an agent exhausting its budget/heartbeat allotment mid-assignment raises a review item instead of declaring done — one paragraph in the Stage 5 role CLAUDE.mds. Prime-agent's daemon+heartbeat+messaging core independently converges on F21-L3 (validating, no action). Adoption of S1/S3 → **D13**.
...
16. Claude Code CLI + per-agent auth (D5); worktrees of the pilot repo for code-touching roles. Per **F21**: require CLI ≥ v2.1.224, keep `DISABLE_TELEMETRY`-family env vars out of agent environments, keep agent sessions on a shared host/container (messaging is filesystem-partitioned), and set the explicit Tier-0 messaging posture in the allowlist presets (deny `SendMessage`/`ListAgents` + `crossSessionInbound: refuse`) until D12 opens it.
...
18. **H-Task 8** early, per F10: Tier-0 allowlists per role in each worktree; everything outside them routes to review (D4). Trust-tier files at 0 in `config/trust_tiers/`. Role CLAUDE.mds gain the F4 stage-setter convention and — pending D13 — F22-S3: budget/heartbeat exhaustion mid-assignment raises a review item, never "done"; per-assignment done names a machine-checkable gate where one exists.
...
## 5. Decisions (D1–D9 resolved 2026-07-23 · D10 resolved 2026-07-24 · D11–D13 open)
...
| D12 | Cross-session messaging (F21) — scope and timing | **OPEN** (raised 2026-08-08). Three sub-questions: (a) enable **L1** (review-answer wake via the H-Task 5 cron posting to the raising agent's session socket) in the pilot at Stage 6, or hold all messaging until H-Task 9 evidence? (b) accept the same-OS-user trust boundary for the single-team pilot (recommended: yes, revisit at multi-team rollout)? (c) tag message-triggered turns in the F13 instrumentation from day one (recommended: yes — enables the L3 comparison)? Independent of this decision, Stage 5 ships the explicit Tier-0 deny/refuse posture, the CLI version floor, and the shared-host constraint (F21 steps 1–3). **L3** (persistent sessions) is not decided here — it's the fourth candidate in the H-Task 9 bake-off (F17/F18 as amended). Research: `harness/docs/research-cross-session-messaging.md`. Default if unreviewed: deny/refuse posture stands, no messaging in the pilot. |
| D13 | prime-agent steals (F22) — bind S1, adopt S3 | **OPEN** (raised 2026-08-09). (a) Make S1's refinement-discipline criteria (evidence-backed diffs, second-model review gate, versioned history, rollback, immutable base prompt, scope isolation) *binding* on the H-Task 9 memory decision, or advisory? (recommended: binding — it is F18 gate 6 made concrete). (b) Adopt S3's "budget exhaustion ≠ success / named quality gates" paragraph in the Stage 5 role CLAUDE.mds? (recommended: yes — no code, closes a real declared-success-on-empty-budget failure mode). (c) Keep or strike agent-authored skills from the long-term watch-list. The prime-agent *runtime* is not on the table (unsandboxed, second orchestrator). Research: `harness/docs/research-prime-agent-patterns.md`. Default if unreviewed: S1 advisory, S3 not added, skills struck. |
