...
- `docs/implementation-plan.md` is the working plan: host topology (dev computer / NAS / appserver), review findings F1–F19 (F17/F18 = the Beads + claude-mem memory-layer watch-list — one H-Task 9 bake-off decision, adopt only on pilot evidence + user sign-off), staged sequencing for both systems, decisions D1–D10 (all resolved — see plan §5). Read it before starting any stage; keep its stage checklist current as work lands.
