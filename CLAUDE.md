...
- `docs/implementation-plan.md` is the working plan: host topology (dev computer / NAS / appserver), review findings F1–F22 (F17/F18/F21-L3 = the memory/re-context watch-list — Beads, claude-mem, persistent sessions + cross-session messaging; **one** H-Task 9 bake-off decision, adopt only on pilot evidence + user sign-off, with F22's refinement-discipline criteria applied to every candidate; F20 = dashboard-gallery patterns, `portfolio/docs/research-obsidian-dashboard-gallery.md`; F21 = cross-session messaging, `harness/docs/research-cross-session-messaging.md`; F22 = prime-agent governance steals, `harness/docs/research-prime-agent-patterns.md`), staged sequencing for both systems, decisions D1–D10 resolved, **D11–D13 open** (F20 dashboard patterns; F21 messaging scope; F22 steals — see plan §5). Read it before starting any stage; keep its stage checklist current as work lands.
...
- Google Chat is a first-class channel: per-agent named webhooks first (one-way), two-way Chat app only as an explicit Phase 2 — plan finding F14. Hard rules #4/#5 in `harness/CLAUDE.md` still apply. The Chat *plumbing* lives in the separate `mmackelprang/chat-gateway` repo (F19/D10, extracted 2026-07-24 for multi-app use — aitrader, job-hunter, this harness): aiteam consumes it strictly as an HTTP service; `notify.py` keeps the renderers and gains a gateway transport at Stage 6; `bin/new-project` provisions comms via its registry.
