...
- `docs/implementation-plan.md` is the working plan: host topology (dev computer / NAS / appserver), review findings F1–F20 (F17/F18 = the Beads + claude-mem memory-layer watch-list — one H-Task 9 bake-off decision, adopt only on pilot evidence + user sign-off; F20 = dashboard-gallery patterns research, `portfolio/docs/research-obsidian-dashboard-gallery.md`), staged sequencing for both systems, decisions D1–D10 resolved, **D11 open** (adopt/review/defer the F20 dashboard patterns — see plan §5). Read it before starting any stage; keep its stage checklist current as work lands.
